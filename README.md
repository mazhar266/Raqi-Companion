...
- **Query tab** — look up any ayah, hadith, or supplication with a compact reference such as `2:255`, `Q:2:1-5,255`, `HM:27:1-3`, or `B:1:1` — the source is optional and a bare reference means the Quran — covering the whole Quran, Hisnul Muslim, and the nine hadith books. Doubling the colon numbers across the whole book instead, so `B::6018` is Bukhari hadith 6018 as it is normally cited. Android only; see below.
...
- **Back up your lists** — export them to a JSON file and restore them later or on another device, from **Settings**. The file holds only the references, so it is a couple of kilobytes and keeps working after the app's data is updated. Android's own backup also carries your lists and bookmarks to a new phone, with no account or setup.
- **Arabic size slider** — scale the Arabic from 80% to 200% in Settings, with a live preview. Applies to the Arabic only; the rest of the interface is unchanged.
...
> the web build bundles 125 MB of query data that a browser can never read. Browsers never
...
The Quran text is [Tanzil](https://tanzil.net/)'s Uthmani edition. It replaced an earlier source that spelled the open tanwin with codepoints meaning something else in Unicode, which made 2:286 read *isru* rather than *isran* in any font that follows the standard.
