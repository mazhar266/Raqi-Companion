...
sources/                           QQL data (125 MB, 6789 files) — Quran, Hisnul Muslim,
...
- **QCF / King Fahd Complex page fonts cannot be used here.** They map ~2,000 Private Use Area codepoints, each a pre-shaped word from one mushaf line, and carry nothing in the Arabic block. They need per-ayah PUA strings (`code_v1`/`code_v2`), which neither Tanzil nor `quran-json-arabic` provides — the `text` is ordinary Unicode. `tool/check_font_coverage.py` reports such a font as missing all 88 codepoints.
- **The Quran text is Tanzil's Uthmani, and `sources/quran/` is generated.** It used to come straight from the `quran-json-arabic` package, which spells three marks with codepoints that mean something else in Unicode (`U+0657` for an open fathatan, `U+065E` dammatan, `U+0656` kasratan). A font following Unicode drew them literally, so 2:286's `إِصْرًا` gained a damma above the reh and read **isru** rather than **isran** — about 6,800 marks across the mushaf. Rebuild it in QQ Lang with `scripts/build-quran.py`, then re-vendor; never hand-edit, and re-run `tool/resync_quran_text.py` afterwards.
- **`normalizeQuranicMarks()` stays as a guard, not a fix.** It maps those three codepoints to the standard tanwin and `arabicText()` applies it to everything, but the bundled data no longer needs it — `test/tajweed_test.dart` asserts zero ayat are rewritten. It earns its place because QQL can be pointed at user-defined JSON sources, which may still carry them.
